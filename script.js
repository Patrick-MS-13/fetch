...
function renderUL() {
  ...
  itemsArray.forEach((item) => {
    ...
    li.id = `item${item.id}`;
    li.className = "new-item"; // Add this class to new items
...
function toggleEditSave(button) {
  ...
  // Ensure only one button (Edit/Save) is enabled
  updateButtonsState();
}
...
function addItem() {
  ...
  if (title) {
    ...
    const newItem = {
      id: itemsArray.length + 1, // Simple way to generate an ID, could be more sophisticated
      title: title
    };
